...
Usage:
    cd notebooks && uv run train_l17_instances_gpu.py

GPU version: Automatically uses CUDA if available.
"""
...
def get_gnn_predictions(model, dataset: list, mean_y: float, std_y: float, device=None) -> List[float]:
    model.eval()
    predictions = []
    with torch.no_grad():
        for data in dataset:
...
def main():
    ...
    for aug_phase_num in range(1, CONFIG["n_augmentation_phases"] + 1):
        ...
        predictions = get_gnn_predictions(model, current_train_dataset, mean_y, std_y, device=device)
